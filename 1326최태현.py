...
REQUIRED_ASSET_FILES = [
    ...
    # 디미고 아이템 아이콘 (존재하지 않아도 오류 없음)
    "dimigo_ticket.png", "dimigo_president.png", "dimigo_teacher.png",
    "dimigo_principal.png", "dimigo_chairman.png",
...
class GameState(Enum):
    ...
    MISSIONS = auto()
    TECH_TREE = auto()
    ENDING_PRINCIPAL = auto() # 교장 엔딩
    ENDING_CHAIRMAN = auto()  # 이사장 엔딩
...
class Game:
    def __init__(self):
        ...
        # 게임 이름 변경
        pygame.display.set_caption("곡괭이로 파낸 디미고 최고 자리")
        ...
        if not self.font:
            print("경고: 한글 폰트(맑은 고딕 등)를 찾지 못해 기본 폰트를 사용합니다.")
            self.font = pygame.font.Font(None, 16) # 👈 20을 16으로 변경
            self.font_small = pygame.font.Font(None, 12) # 👈 이 줄 추가
            self.font_big = pygame.font.Font(None, 24) # 👈 이 줄 추가
        ...
        self.previous_state = None # 설명서 보기 이전 상태 저장
    ...
    def _load_scaled_image(self, file_name, size):
        ...
        if not os.path.exists(file_path):
            # print(f"에셋 로드 실패: {file_path}") # 디버깅용
            return None
    ...
    def load_assets(self):
        ...
        name_map = {
            ...
            # 디미고 아이템 아이콘 매핑 (파일이 없으면 기본 도형으로 그려짐)
            ItemType.DIMIGO_TICKET: "dimigo_ticket.png",
            ItemType.DIMIGO_PRESIDENT: "dimigo_president.png",
            ItemType.DIMIGO_TEACHER: "dimigo_teacher.png",
            ItemType.DIMIGO_PRINCIPAL: "dimigo_principal.png",
            ItemType.DIMIGO_CHAIRMAN: "dimigo_chairman.png",
        }
    ...
    def run(self):
        while True:
            ...
            # INTRO, ENDING 상태일 때는 world.tick()을 실행하지 않음
            if self.state in [GameState.PLAY, GameState.CONTAINER, GameState.SHOP, GameState.TECH_TREE, GameState.MISSIONS]:
                self.logic_timer += dt
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_e:
                    ...
                    elif self.state == GameState.PLAY: self.open_container(None)
                ...
                # 'H' 키로 설명서 열기/닫기
                    ...
                    if self.state == GameState.INTRO: # 설명서 화면에서
                        if self.previous_state:
                            self.state = self.previous_state # 이전 상태로 복귀
                        else:
                            self.state = GameState.PLAY # 비상시
                        self.previous_state = None
                    elif self.state not in [GameState.INTRO, GameState.ENDING_PRINCIPAL, GameState.ENDING_CHAIRMAN]: # 다른 화면에서
                        self.previous_state = self.state # 현재 상태 저장
                        self.state = GameState.INTRO # 설명서 열기
                
                elif event.key == pygame.K_ESCAPE:
                    if self.state == GameState.INTRO:
                        if self.previous_state: # 게임 중 설명서를 연 경우
                            self.state = self.previous_state
                            self.previous_state = None
                        # 게임 첫 시작(previous_state=None)이면 ESC 무시
                    elif self.state in [GameState.ENDING_PRINCIPAL, GameState.ENDING_CHAIRMAN]:
                        pass # 엔딩 화면에서는 ESC 무시
                    elif self.state != GameState.PLAY:
                        self.close_container()
            ...
            if event.type == pygame.MOUSEWHEEL:
                ...
                # 핫바 휠 스크롤
                elif self.state in [GameState.PLAY, GameState.CONTAINER, GameState.TECH_TREE]:
                    # event.y > 0 (위로) 이면 인덱스 감소
                    # event.y < 0 (아래로) 이면 인덱스 증가
                    new_idx = (self.selected_hotbar_idx - event.y) % 9
                    self.selected_hotbar_idx = new_idx

            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.state == GameState.INTRO:
                    if event.button == 1: self.handle_intro_click(mx, my)
                elif self.state in [GameState.ENDING_PRINCIPAL, GameState.ENDING_CHAIRMAN]:
                    if event.button == 1: self.handle_ending_click(mx, my)
    ...
    def get_slot_under_mouse(self, mx, my):
        # 핫바 휠 스크롤 대상이므로 CONATAINER, TECH_TREE 외에 PLAY에서도 슬롯 감지
        if self.state not in [GameState.PLAY, GameState.CONTAINER, GameState.TECH_TREE]: return None
        
        # 인벤토리 (하단)
        if self.state in [GameState.CONTAINER, GameState.TECH_TREE]:
        ...
        if self.state == GameState.PLAY:
             ...
             for i in range(9):
                sx, sy = px + i * 44, py
                if pygame.Rect(sx, sy, 40, 40).collidepoint(mx, my): return (self.world.player_inv, 27 + i)

        # 열린 컨테이너 (상단)
        if self.state in [GameState.CONTAINER, GameState.TECH_TREE] and self.opened_container:
            c_x, c_y = SCREEN_WIDTH // 2, 100
    ...
    def handle_shop_click(self, mx, my):
        ...
        if buy_viewport.collidepoint(mx, my):
            ...
            if 0 <= item_idx < len(SHOP_ITEMS):
                if adjusted_y % 60 <= 50:
                    ...
                    # '???' 아이템 특별 로직
                    is_chairman = item_type == ItemType.DIMIGO_CHAIRMAN
                    if is_chairman and item_type not in self.world.purchased_upgrades:
                        if self.world.buy_upgrade(item_type):
                            self.ending_message = ["진정한 디미고의 주인이 되었습니다."]
                            self.state = GameState.ENDING_CHAIRMAN
                        return
                    ...
                    if not is_locked:
                        # 업그레이드 아이템 구매
                        if item_data.get("is_upgrade"):
                            if item_type not in self.world.purchased_upgrades:
                                if self.world.buy_upgrade(item_type):
                                    # 엔딩 트리거 확인
                                    if item_type == ItemType.DIMIGO_PRINCIPAL:
                                        self.ending_message = ["당신은 디미고의 1짱이 되었습니다."]
                                        self.state = GameState.ENDING_PRINCIPAL
                                    elif item_type == ItemType.DIMIGO_CHAIRMAN:
                                        self.ending_message = ["진정한 디미고의 주인이 되었습니다."]
                                        self.state = GameState.ENDING_CHAIRMAN
                        # 건물 아이템 구매
                        elif item_data.get("is_building"):
                            cost = item_data["cost"]
        ...
        # 자원 판매 로직
        sell_items = [t for t in ItemType if not ITEM_DATA[t].get("is_building") and not ITEM_DATA[t].get("is_upgrade")]
        ...
        if sell_all_rect.collidepoint(mx, my):
            for i, stack in enumerate(self.world.player_inv.slots):
                if stack:
                    item_data = ITEM_DATA[stack.item_type]
                    if not item_data.get("is_building") and not item_data.get("is_upgrade"):
                        self.world.money += item_data["value"] * stack.count
    ...
    # 인트로 클릭 핸들러 (수정)
    def handle_intro_click(self, mx, my):
        btn_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT - 100, 200, 50)
        if btn_rect.collidepoint(mx, my):
            if self.previous_state:
                self.state = self.previous_state # 이전 상태로 복귀
            else:
                self.state = GameState.PLAY # 게임 시작
            self.previous_state = None # 상태 복귀 후 초기화

    # 엔딩 클릭 핸들러 (새 메서드)
    def handle_ending_click(self, mx, my):
        btn_rect = pygame.Rect(SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT - 100, 300, 50)
        if btn_rect.collidepoint(mx, my):
            self.state = GameState.PLAY
            self.ending_message = []
    ...
    def render(self):
        ...
        # 새 게임 상태 렌더링
        if self.state == GameState.INTRO:
            self.render_intro_ui()
            pygame.display.flip()
            return
        elif self.state in [GameState.ENDING_PRINCIPAL, GameState.ENDING_CHAIRMAN]:
            self.render_ending_ui()
            pygame.display.flip()
            return
        ...
        if self.held_item:
            ...
            # 마우스 휠 스크롤 중이 아닐 때만 툴팁 표시 (휠 중에는 마우스 위치가 고정됨)
            if not any(pygame.mouse.get_pressed()):
                slot_info = self.get_slot_under_mouse(mx, my)
                if slot_info:
                    inv, idx = slot_info
                    if inv.slots[idx]:
                        tooltip_stack = inv.slots[idx]
    ...
    def render_hud(self):
        ...
        # [H]설명서 추가
        info = f"자금: {self.world.money:,.0f} Cr | [E]인벤 [B]상점 [M]미션 [H]설명서 | [F]줍기 [우클릭]상호작용"
        self.screen.blit(self.font.render(info, True, COLOR_TEXT), (20, 10))
        
        # 패시브 인컴 표시
        if self.world.passive_income_per_sec > 0:
            income_text = self.font.render(f"(+{self.world.passive_income_per_sec:,.0f} Cr/s)", True, COLOR_ARROW)
            info_width = self.font.size(info)[0]
            self.screen.blit(income_text, (20 + info_width + 10, 10))
    ...
    def render_shop_ui(self):
        ...
        # 돈, 패시브 인컴 표시
        self.screen.blit(self.font_big.render(f"보유: {self.world.money:,.0f} Cr", True, COLOR_ARROW), (SCREEN_WIDTH - 300, 30))
        self.screen.blit(self.font.render(f"초당 수입: +{self.world.passive_income_per_sec:,.0f} Cr/s", True, COLOR_ARROW), (SCREEN_WIDTH - 300, 70))


        mx, my = pygame.mouse.get_pos()
        
        buy_viewport = pygame.Rect(50, 140, 300, SCREEN_HEIGHT - 200)
        self.screen.blit(self.font_big.render("[ 건물/업그레이드 ]", True, COLOR_TEXT), (50, 80))
        ...
        for item_type in SHOP_ITEMS:
            ...
            if r.bottom > buy_viewport.top and r.top < buy_viewport.bottom:
                ...
                item_data = ITEM_DATA[item_type]
                is_upgrade = item_data.get("is_upgrade", False)
                is_purchased = item_type in self.world.purchased_upgrades

                # '디미고 이사장' 숨김 처리
                is_chairman = item_type == ItemType.DIMIGO_CHAIRMAN
                if is_chairman and not is_purchased:
                    col = COLOR_BUTTON_LOCKED
                    if r.collidepoint(mx, my) and buy_viewport.collidepoint(mx, my): col = COLOR_BUTTON_HOVER
                    pygame.draw.rect(self.screen, col, r, border_radius=8)
                    self.screen.blit(self.font.render(f"디미고 ??? - {item_data['cost']:,.0f} Cr", True, (255, 0, 255)), (100, y + 15))
                    y += 60
                    continue
                ...
                col = COLOR_BUTTON
                if is_locked: col = COLOR_BUTTON_LOCKED
                elif is_purchased: col = COLOR_BUTTON_RESEARCHED # 구매 완료 색상
                elif r.collidepoint(mx, my) and buy_viewport.collidepoint(mx, my): col = COLOR_BUTTON_HOVER
                ...
                cost_str = f"{item_data['cost']:,.0f} Cr"
                if is_purchased:
                    cost_str = "[구매 완료]"
                    txt_col = (200, 255, 200)

                self.screen.blit(self.font.render(f"{item_data['name']} - {cost_str} {lock_msg}", True, txt_col), (100, y + 15))
            y += 60
        ...
        self.screen.blit(self.font.render("모두 판매 (자원만)", True, COLOR_TEXT), (410, SCREEN_HEIGHT - 100 + 15))
        ...
        for mission_id in self.world.available_missions:
            ...
            if r.bottom > mission_viewport.top and r.top < mission_viewport.bottom:
                ...
                if "money" in data["rewards"]:
                    # 돈 포맷 변경
                    self.screen.blit(self.font.render(f"{data['rewards']['money']:,.0f} Cr", True, (255, 215, 0)), (rew_x, y + 55))
                    rew_x += 100
    ...
    # 인트로 UI 렌더링 (수정됨)
    def render_intro_ui(self):
        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        overlay.fill((0,0,0,220))
        self.screen.blit(overlay, (0,0))
        
        # Y위치 80 -> 60으로 수정
        title_text = self.font_big.render("곡괭이로 파낸 디미고 최고 자리", True, (255, 215, 0))
        title_rect = title_text.get_rect(center=(SCREEN_WIDTH // 2, 60)) 
        self.screen.blit(title_text, title_rect)

        intro_lines = [
            "게임 설명서 (H키 또는 ESC로 닫기)",
            "",
            "당신은 이 황무지에서 공장을 건설해야 합니다.",
            "자원을 채굴하고(채굴기), 제련하고(용해로), 조립하여(제작기) 자동화 공장을 만드세요.",
            "생산된 자원은 [B]상점의 [자원 판매] 탭에서 팔아 돈을 벌 수 있습니다.",
            "",
            "[ 조작키 ]",
            " - W, A, S, D: 카메라 이동",
            " - 1-9 또는 마우스 휠: 핫바 슬롯 선택",
            " - R: 건물 회전",
            " - F: 주변 아이템 줍기",
            " - X: 삭제 모드 (건물 철거)",
            " - E: 인벤토리 열기",
            ...
            " - H: 설명서 열기",
            " - T: 연구 트리 열기 (연구소 우클릭)",
            " - 마우스 좌클릭: 건물 설치 / UI 상호작용",
            " - 마우스 우클릭: 건물 정보 확인 / 상호작용", # 줄바꿈
            "   (보관함, 연구소 등)", # 들여쓰기
            "",
            "[ 목표 ]",
            "돈을 벌어 상점에서 '디미고' 관련 직책을 구매하세요.",
            "최종 목표는 '디미고 교장'과 그 너머에 있습니다.",
            "행운을 빕니다!"
        ]

        y = 90  # Y 시작 위치 130 -> 110으로 수정
        # line_height = 19  # 줄 간격 20 -> 19로 수정 (이 줄을 수정합니다)
        line_height = self.font.get_linesize() # 텍스트를 정렬할 기준 X 위치 (화면 중앙에서 왼쪽으로)
        align_x = SCREEN_WIDTH // 2 - 280 

        for line in intro_lines:
            text_surf = self.font.render(line, True, COLOR_TEXT)
            
            # 정렬 로직
            if line.startswith("[") or line == "" or line.startswith("게임 설명서") or line.startswith("최종") or line.startswith("행운을"):
                text_rect = text_surf.get_rect(midtop=(SCREEN_WIDTH // 2, y)) # 👈 2. 정렬 버그 수정
            elif line.startswith("   ("):
                # 들여쓰기가 필요한 줄
                text_rect = text_surf.get_rect(topleft=(align_x + 20, y))
            else:
                # 나머지 모든 줄은 왼쪽 정렬
                text_rect = text_surf.get_rect(topleft=(align_x, y))
        ...
        # 버튼 (시작 / 돌아가기)
        mx, my = pygame.mouse.get_pos()
        btn_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT - 100, 200, 50) # y=620
        col = COLOR_BUTTON_HOVER if btn_rect.collidepoint(mx, my) else COLOR_BUTTON
        pygame.draw.rect(self.screen, col, btn_rect, border_radius=8)
        
        button_label = "돌아가기" if self.previous_state else "게임 시작"
        btn_text = self.font_big.render(button_label, True, COLOR_TEXT)
        btn_text_rect = btn_text.get_rect(center=btn_rect.center)
        self.screen.blit(btn_text, btn_text_rect)

    # 엔딩 UI 렌더링 (새 메서드)
    def render_ending_ui(self):
        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        overlay.fill((0,0,0,220))
        self.screen.blit(overlay, (0,0))
        
        y = SCREEN_HEIGHT // 2 - 100
        for line in self.ending_message:
            title_text = self.font_big.render(line, True, (255, 215, 0))
            title_rect = title_text.get_rect(center=(SCREEN_WIDTH // 2, y))
            self.screen.blit(title_text, title_rect)
            y += 40
        ...
        col = COLOR_BUTTON_HOVER if btn_rect.collidepoint(mx, my) else COLOR_BUTTON
        pygame.draw.rect(self.screen, col, btn_rect, border_radius=8)
        btn_text = self.font_big.render("계속 진행하기", True, COLOR_TEXT)
        btn_text_rect = btn_text.get_rect(center=btn_rect.center)
        self.screen.blit(btn_text, btn_text_rect)
    ...
    def render_tooltip(self, stack, mx, my):
        ...
        # 패시브 인컴 툴팁 (이미 구매한 아이템 툴팁에는 안 뜨게)
        if data.get("is_upgrade") and stack.item_type not in self.world.purchased_upgrades:
             lines.append(f"구매 시: +{data.get('passive_income', 0)} Cr/s")
        elif data.get("is_upgrade") and stack.item_type in self.world.purchased_upgrades:
             lines.append(f"[구매 완료]")
    ...
    def draw_item(self, x, y, item_type, count=1):
        ...
        if sprite: self.screen.blit(sprite, (x + 2, y + 2))
            ...
            # 업그레이드 아이템은 별 모양(사각형)으로 표시
            if ITEM_DATA[item_type].get("is_building") or ITEM_DATA[item_type].get("is_upgrade"):
